#ifndef B28D145E_0F50_427C_9F74_B7605DF90DAF
#define B28D145E_0F50_427C_9F74_B7605DF90DAF
#include <atomic>
#include <cmath>
#include <iostream>
#include <mutex>
...
class RangeAccumulator {
 public:
  ...
  static void AtomicAccumulatorRelaxed(unsigned long low, unsigned long high) {
    for (unsigned long i = low; i < high; i++) {
      benchmark::DoNotOptimize(
          _atomic_sum.fetch_add(i, std::memory_order_relaxed));
    }
  }
  //-----------------------------------------------------
  // Accumulates values from low to high using reduction and relaxed memory
  // order
  static void AtomicAccumulatorPartition(unsigned long low,
                                         unsigned long high) {
  ...
  // Accumulates values from low to high using reduction and relaxed memory
  // order
  static void AtomicAccumulatorPartitionRelaxed(unsigned long low,
                                                unsigned long high) {
    unsigned long local_sum = 0;
    for (unsigned long i = low; i < high; i++) {
      benchmark::DoNotOptimize(local_sum += i);
  ...
  //-----------------------------------------------------
  template <class T>
  static void Driver(unsigned long number_of_threads, T func,
                     unsigned long size) {
    Init();
    ...
    unsigned long num_chunks =
        step == 0 ? 1 : std::ceil((double)size / (double)step);
...
#endif /* B28D145E_0F50_427C_9F74_B7605DF90DAF */
