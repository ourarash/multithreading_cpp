#ifndef B70344C7_AE02_460A_926A_75DE4F08B159
#define B70344C7_AE02_460A_926A_75DE4F08B159
#include <atomic>
#include <cmath>
#include <iostream>
#include <mutex>
...
class VectorAccumulator {
 public:
  ...
  static void AtomicAccumulator(std::vector<unsigned long> &a,
  ...
  //-----------------------------------------------------
  static void AtomicAccumulatorRelaxed(std::vector<unsigned long> &a,
                                       unsigned long low, unsigned long high) {
    for (unsigned long i = low; i < high; i++) {
  ...
  //-----------------------------------------------------
  static void AtomicAccumulatorPartitionRelaxed(std::vector<unsigned long> &a,
                                                unsigned long low,
                                                unsigned long high) {
  ...
  static void MutexAccumulatorPartition(std::vector<unsigned long> &a,
                                        unsigned long low, unsigned long high) {
    unsigned long local_sum = 0;
    for (unsigned long i = low; i < high; i++) {
      local_sum += a[i];
  ...
  //-----------------------------------------------------
  template <class T>
  static void Driver(std::vector<unsigned long> &a,
                     unsigned long number_of_threads, T func) {
    Init();
    std::vector<std::thread> threads;

    unsigned long step = a.size() / number_of_threads;
    unsigned long num_chunks =
        step == 0 ? 1 : std::ceil((double)a.size() / (double)step);
    ...
    for (unsigned long i = 0; i < num_chunks; i++) {
      ...
      threads.push_back(
          std::thread([&a, &func, low, high]() { func(a, low, high); }));
    }
...
#endif /* B70344C7_AE02_460A_926A_75DE4F08B159 */
