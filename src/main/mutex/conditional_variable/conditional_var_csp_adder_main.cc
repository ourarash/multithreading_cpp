...
int main() {
  ...
  threads.push_back(std::thread(producer, std::ref(op1), /*delay_ms=*/200));
  threads.push_back(std::thread(producer, std::ref(op2), /*delay_ms=*/200));

  threads.push_back(std::thread(adder, std::ref(op1), std::ref(op2),
                                std::ref(result),
                                /*delay_ms=*/50));

  threads.push_back(std::thread(consumer, std::ref(result), /*delay_ms=*/500));

  threads.push_back(
      std::thread(monitor, std::ref(op1), std::ref(op2), std::ref(result)));
